...
export async function runStartupCheck(stateDir: string): Promise<void> {
  const lockFile = path.join(stateDir, "session-start.lock");
  try {
    const stat = await fs.stat(lockFile);
    if (Date.now() - stat.mtimeMs < SESSION_LOCK_TTL_MS) return;
  } catch {
...
async function doStartupCheck(stateDir: string, lockFile: string): Promise<void> {
  ...
  try {
    const required = await fs.readFile(
      path.join(resourceDir(), "copilot-instructions.md"),
      "utf8"
    );
  ...
  if (warnings.length > 0) {
    process.stderr.write(
      `⚠  Memory Hook: setup incomplete — copy the files from resource/ to ~/.copilot/:\n${warnings.join("\n")}\n`
    );
  }
