...
interface MemoryListItem {
  time: number;
  type: "user_prompt" | "ask_user" | "user_responed";
  content: string;
}

function candidateSummary(candidate: MemoryCandidate): Record<string, unknown> {
  return {
    id: candidate.id,
    kind: candidate.kind,
    hook: candidate.hook,
...
function timestamp(input: { timestamp?: number }): number {
  return typeof input.timestamp === "number" ? input.timestamp : Date.now();
}

function cwd(input: { cwd?: string }): string {
...
function textFromUnknown(value: unknown): string {
  ...
  if (direct) return textFromUnknown(direct);
  if (record.questions) return textFromUnknown(record.questions);
  return "";
}
...
function item(time: number, type: MemoryListItem["type"], content: string): MemoryListItem | undefined {
  const cleaned = singleLine(redactString(content));
  return cleaned ? { time, type, content: cleaned } : undefined;
}

function askUserText(input: PostToolUseInput): string {
  return textFromUnknown(parseToolArgs(input.toolArgs));
}

function userResponseText(input: PostToolUseInput): string {
...
async function recordItems(
  config: HookConfig,
  hook: HookName,
  input: { timestamp?: number; cwd?: string },
...
function itemsFromEvents(events: FlowEvent[]): MemoryListItem[] {
  const ordered = [...events].sort((a, b) => a.timestamp - b.timestamp);
  const items = ordered.flatMap((event) => {
    const rawItems = event.data.items;
    return Array.isArray(rawItems) ? rawItems as MemoryListItem[] : [];
  });

  const seen = new Set<string>();
  const unique: MemoryListItem[] = [];
...
function makeFlowCandidate(
  events: FlowEvent[],
  terminalInput: SessionEndInput
): MemoryCandidate | undefined {
  const payload = itemsFromEvents(events);
  const evidence = JSON.stringify(payload);
  if (!payload.length || hasSensitiveContent(evidence)) return undefined;
...
async function ingestAndMark(
  ...
  await debugLog(
    config.stateDir,
    `ingest failed for ${candidate.evidenceHash}: ${result.error ?? "unknown error"}`
  );
...
async function flushFlowAtAgentStop(config: HookConfig, input: SessionEndInput): Promise<void> {
...
export async function handlePostToolUse(
  ...
  if (input.toolName === "ask_user") {
    ...
    if (responseItem) items.push(responseItem);
...
export async function handleHook(
  ...
  if (hookName === "userPromptSubmitted") {
    await handleUserPromptSubmitted(input as UserPromptSubmittedInput, config);
    return;
  }

  if (hookName === "postToolUse") {
    await handlePostToolUse(input as PostToolUseInput, config);
    return;
  }

  if (hookName === "agentStop") {
    await handleAgentStop(input as SessionEndInput, config);
  }
}
